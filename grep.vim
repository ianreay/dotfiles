...
if !exists("Grep_Skip_Files")
    let Grep_Skip_Files = '*~ *,v s.*, *.bin, *.obj'
endif
...
function! s:RunGrepRecursive(cmd_name, grep_cmd, action, ...)
    ...
    while txt != ''
        ...
        let find_file_pattern = find_file_pattern . ' -iname ' .
              \ g:Grep_Shell_Quote_Char . one_pattern . g:Grep_Shell_Quote_Char
        let txt = strpart(txt, stridx(txt, ' ') + 1)
    endwhile
    let find_file_pattern = g:Grep_Shell_Escape_Char . '(' . g:Grep_Shell_Escape_Char .
                    \ find_file_pattern . ' ' . g:Grep_Shell_Escape_Char . ')' . g:Grep_Shell_Escape_Char
...
command! -nargs=* -complete=file Igrep
            \ call s:RunGrepRecursive('Rgrep', 'grep', 'set', <f-args>)
command! -nargs=* -complete=file Ygrep
            \ call s:RunGrepRecursive('Rgrep', 'grep', 'set', <f-args>)
